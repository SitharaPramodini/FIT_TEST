            ...
            {services[index].includes.map((includes, index) =>
              <div className="mr-auto" key={index}>
                ...
                >
                  ...
                  {includes.features.length <= 3 ? (
  // Single column list for 3 or fewer features
  <ul className="flex flex-col">
    {includes.features.map((feature, i) => (
      <li className="text-md mt-3 text-white list-disc ml-8" key={i}>
        {feature}
      </li>
    ))}
  </ul>
) : (
  // Two-column list for more than 3 features
  <div className="flex flex-row gap-x-4">
    <ul className="flex-1 flex-col">
      {includes.features.slice(0, Math.ceil(includes.features.length / 2)).map((feature, i) => (
        <li className="text-md mt-3 w-[80%] text-white list-disc ml-8" key={i}>
          {feature}
        </li>
      ))}
    </ul>
    <ul className="flex-1 flex-col">
      {includes.features.slice(Math.ceil(includes.features.length / 2)).map((feature, i) => (
        <li className="text-md mt-3 w-[80%] text-white list-disc ml-8" key={i + 'b'}>
          {feature}
        </li>
      ))}
    </ul>
